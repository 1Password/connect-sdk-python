...
from onepasswordconnectsdk.utils import is_valid_uuid
...
def new_client(url: str, token: str, is_async: bool = False):
    ...
    if is_async:
        ...
        return AsyncClient(session=session, serializer=serializer)

    session = httpx.Client(base_url=url, headers=headers)
    return Client(session=session, serializer=serializer)
